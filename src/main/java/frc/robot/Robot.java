    ...
    switch (m_autoSelected) {
      ...
      default:
        ...
        {     //drive forward 7 ft
                ...
                backServo.setPosition(1);//arm control

        } 
            else if(mTimer.get() < 3.15)// turn 90 deg right
        
        {     
                mDrive.tankDrive(.7, .7);

        }
            else if(mTimer.get() < 6.05)//back up or drive to the goal
         
        { 
                 mDrive.tankDrive(-.5, -.5);
        }
            else if (mTimer.get() < 8.05) // move servo out
